...
export const getProfile = async (username) => {
  try {
    // GET 요청으로 데이터를 가져옵니다.
    let response = await axios.get(`${API_BASE_URL}/profile`, {
      params: { username }  // 쿼리 파라미터로 username 전달
    });
...
// profiledb에 데이터 저장하기 (PUT 요청)
export const updateProfile = (editData, username) => {
  return axios.put(`${API_BASE_URL}/profile`, { ...editData, username })
    .then(response => response.data)
    .catch(error => {
      console.error('데이터 저장 실패:', error);
      throw error;
